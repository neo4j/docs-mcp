...
- MCP requests need the `Content-Type: application/json` header
...
[IMPORTANT]
====
The certificate's Common Name (CN) and Subject Alternative Names (SANs) must match the domain name clients use to connect.
Let's Encrypt certificates automatically include the correct domain names.
====
...
- xref:configuration.adoc#transport-modes[Transport modes]
